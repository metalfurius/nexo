...
export function publicCatalogDraftFromCandidate(candidate: DiscoveryCandidate, timestamp = nowIso()): PublicCatalogItem {
  ...
  return {
    ...
    progressTotal: candidate.progressTotal ?? snapshot?.progressTotal,
    progressUnit: candidate.progressUnit ?? snapshot?.progressUnit,
...
export function getCatalogRepairDraft(
  ...
  nextItem.searchTokens = createSearchTokens({
    ...
    searchAliases: nextItem.searchAliases,
    tags: nextItem.tags,
