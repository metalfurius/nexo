...
test('shell navigation keeps clear labels without responsive overflow', async ({ page }) => {
  ...
  const desktopShellGeometry = await page.evaluate(() => {
    ...
    return {
      ...
      navLeft: tabbar?.getBoundingClientRect().left ?? 0,
      navPosition: tabbar ? window.getComputedStyle(tabbar).position : '',
      navTop: tabbar?.getBoundingClientRect().top ?? 0,
  ...
  expect(desktopScrolledShellGeometry.left).toBe(desktopShellGeometry.navLeft)
  expect(desktopScrolledShellGeometry.top).toBeLessThanOrEqual(1)
...
test('library and weighted dice work in demo mode', async ({ page }) => {
  ...
  await expect(page.getByText('Vista: Sin contexto')).toBeVisible()
  await expect(page.getByTestId('library-filter-summary')).toContainText('Mostrando 4 de 4 filtradas / 7 totales')
  ...
  await expect(page.getByTestId('library-filter-summary')).toContainText('Mostrando 0 de 0 filtradas / 7 totales')
...
test('moderator curation can create a public catalog item in demo mode', async ({ page }) => {
  ...
  const createdDuneCard = page.getByRole('article').filter({ hasText: 'Politica, desierto y destino.' })
  await expect(page.getByRole('heading', { name: 'Solaris' })).toBeVisible()
  await expect(createdDuneCard.getByRole('heading', { name: 'Dune' })).toBeVisible()
